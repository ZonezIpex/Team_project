...
import api from '../services/api'; // Axios 인스턴스 (경로 확인!)
...
// 텍스트 객체 (국제화 지원)
const textContent = {
  CreatingResume: { ko: "AI가 이력서를 분석하고 생성 중입니다...\n잠시만 기다려 주세요.", en: "AI is analyzing and creating your resume...\nPlease wait a moment." },
  ProcessingError: { ko: "처리 중 오류 발생", en: "Error during processing" }
};

function AiGeneratingLoader({ language = "ko" }) { // App.js의 Route에서 language prop을 받음
  ...
  const getText = (sectionKey) => {
    return textContent[sectionKey]?.[currentLanguage] || textContent[sectionKey]?.ko || "AI 처리 중...";
  };
    ...
    const callAiGenerationAPI = async () => {
      ...
      try {
        if (inputPayload.type === "formInput") {
          ...
          console.log("AI API (/generate-from-form)로 보낼 sections:", sectionsForApi);
          ...
          console.log(`AI API (/suggest-from-resume/${userResumeId}) 호출 중...`);

          apiResponse = await api.post(`/api/coverletter/suggest-from-resume/${userResumeId}`);
          // 이 경우, inputPayload.originalData (FileScanningLoader가 전달한 초기 formData)를
          // Step5Page로 그대로 전달하여 원본 정보로 활용 가능

        } else {
          throw new Error("알 수 없는 AI 처리 요청 타입입니다: " + inputPayload.type);
        }
        ...
        if (isMounted) {
          if (apiResponse && apiResponse.data && apiResponse.data.letter) {
            ...
            setAiError('AI 응답을 받았으나, 예상된 형식이 아닙니다 (응답에 letter 필드 누락).');
          }
        }
      } catch (err) {
        ...
        if (isMounted) {
          ...
          } else if (err.message) { // Network Error 등 axios 자체 에러
            message = err.message;
          }
          setAiError(message);
        }
  ...
  useEffect(() => {
    if (!processing) { // API 호출이 완료되었을 때만 실행
      const timer = setTimeout(() => {
        if (navigate) { // navigate 함수 유효성 체크 (테스트 환경 등에서 없을 수 있음)
          // Step5Page로 전달할 데이터 구성
          // originalData는 AI 처리 전의 원본 데이터 (formData 또는 FileScanningLoader가 전달한 originalFormData)
          const originalDataForStep5 = (inputPayload?.type === "formInput") 
                                        ? inputPayload.data 
          ...
          navigate('/step5page', { // App.js에 정의된 Step5Page 라우트 경로
            state: {
              originalData: originalDataForStep5,
              suggestion: aiSuggestion,
              error: aiError,
      ...
      }, aiError ? 3000 : 1500); // 에러 시 메시지를 좀 더 보여주고, 성공 시는 빠르게 이동
  ...
  return (
    <LoaderWrapper>
      ...
      {aiError ? (
        <ErrorMessage>{getText("ProcessingError")}: {aiError}</ErrorMessage>
      ) : (
        // 메시지를 pre-wrap으로 처리하여 \n 줄바꿈 반영
        <Message style={{ whiteSpace: 'pre-wrap' }}>{getText("CreatingResume")}</Message>
      )}
    </LoaderWrapper>
  );
}
